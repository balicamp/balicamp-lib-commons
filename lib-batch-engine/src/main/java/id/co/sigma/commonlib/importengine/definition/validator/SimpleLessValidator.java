 ...
 * @version $Id
 **/
public class SimpleLessValidator<DATA> extends BaseSimpleFieldValidator<DATA>{
	...
	public SimpleLessValidator(Date comparatorValue){
		super(); 
	...
	@SuppressWarnings("unchecked")
	public SimpleLessValidator(BigDecimal comparatorValue){
		super();  
		comparatorData = (DATA) comparatorValue ;
	...
	public boolean validate(DATA data) {
		...
		return CompareResult.less.getInternalRepresentation()==rsl.getInternalRepresentation();
	}
	@Override
	public String generateErrorMessage(DATA data) {
		return "fail to validate :" + data + ", should be less   :" + comparatorData; 
	}
