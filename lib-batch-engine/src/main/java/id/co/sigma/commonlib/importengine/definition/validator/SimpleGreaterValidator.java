 ...
 * @version $Id
 **/
public class SimpleGreaterValidator<DATA> extends BaseSimpleFieldValidator<DATA>{
	...
	public SimpleGreaterValidator(Date comparatorValue){
		super(); 
	...
	@SuppressWarnings("unchecked")
	public SimpleGreaterValidator(BigDecimal comparatorValue){
		super();  
		comparatorData = (DATA) comparatorValue ;
	...
	public boolean validate(DATA data) {
		...
		return CompareResult.greater.getInternalRepresentation()==rsl.getInternalRepresentation()||CompareResult.equal.getInternalRepresentation()==rsl.getInternalRepresentation();
	}

	@Override
	public String generateErrorMessage(DATA data) {
		return "fail to validate :" + data + ", should be greater  :" + comparatorData; 
	}
}
